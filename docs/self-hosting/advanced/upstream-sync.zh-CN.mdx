...
<Steps>
  ...
  假设 云智助手 容器的名称是 `lobe-chat`，使用以下指令停止并删除当前运行的 云智助手 容器：
...
<Callout type={'tip'}>
  **重新部署的话，我本地的聊天记录会丢失吗？**
  ...
  放心，不会的。云智助手 的聊天记录全部都存储在你的本地浏览器中。因此使用 Docker 重新部署 云智助手 时，你的聊天记录并不会丢失。
</Callout>
