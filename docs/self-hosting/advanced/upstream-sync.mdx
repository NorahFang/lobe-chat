...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest 云智助手 image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running 云智助手 Container

  Assuming the 云智助手 container is named `lobe-chat`, use the following commands to stop and remove the currently running 云智助手 container:
  ...
  ### Pull the Latest 云智助手 Image

  Use the following command to pull the latest Docker image for 云智助手:
  ...
  Redeploy the 云智助手 container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of 云智助手's chat records are stored in your local browser. Therefore, when redeploying 云智助手 using Docker, your chat records will not be lost.
</Callout>
